...
{
    CK_ATTRIBUTE *am;
    bool non_pss_found = false;
    CK_MECHANISM_TYPE *allowed;
    int am_nmechs;

    CK_KEY_TYPE type = p11prov_obj_get_key_type(obj);
    switch (type) {
    case CKK_RSA:
        ...
        if (non_pss_found) {
            ...
            return (char *)P11PROV_NAME_RSAPSS;
        }
        break;
    case CKK_EC:
        return (char *)P11PROV_NAME_EC;
        break;
    case CKK_EC_EDWARDS:
                                             ...
                                             OSSL_CALLBACK *cb_fn, void *cb_arg)
{
    ...
    attrs[2].type = CKA_ALLOWED_MECHANISMS;
     ...
     * ALLOWED_MECHANISMS), but the PKCS#11 really supports only MGF1 so we
     * can try to use this as a sing for OpenSSL that this is a PSS key */
    if (attrs[2].pValue != NULL && attrs[2].ulValueLen != 0) {
        params[n++] = OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_RSA_MASKGENFUNC,
                                                       (char *)SN_mgf1, strlen(SN_mgf1));
    }
